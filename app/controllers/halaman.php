...
class Halaman extends Controller
{
    ...
    {
        $tipe = $_POST['tipe'];
        $this->db->query("SELECT * FROM kategori_layanan WHERE kategori ='$tipe' ORDER BY brand ASC");
        $kategorippob = $this->db->resultSet();

        echo '<option value="">Pilih Salah Satu</option>';
        foreach ($kategorippob as $katppob) {
            echo '<option value="' . $katppob['brand'] . '#' . $katppob['tipe'] . '">' . $katppob['brand'] . ' ' .  $katppob['tipe'] . '</option>';
        }
    }
    ...
    {
        $tipe = $_POST['tipe'];
        $operator = explode('#', $_POST['operator'])[0];
        $kategori = explode('#', $_POST['operator'])[1];
        $this->db->query("SELECT * FROM layanan_pulsa WHERE tipe ='$tipe' AND operator = '$operator' AND kategori = '$kategori' ORDER BY harga ASC ");
