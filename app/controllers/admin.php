...
class Admin extends Controller
{
    ...
    public function updatedeposit()
    {
        if (!isset($_POST['status'])) {
            ...
            if ($updatedeposit->rowCount() > 0) {
                $updatesaldo = $this->db->prepare("UPDATE users SET saldo_top_up = '$totalsaldo' WHERE username = '$username' ");
                $updatesaldo->execute();
    ...
    public function addnews()
    {
        ...
        $kategori = rtrim(filter($_POST['kategori']));
        $tipe = rtrim(filter($_POST['tipe']));
        $title = rtrim(filter($_POST['title']));
        $konten = rtrim(filter($_POST['konten']));
        $date = $this->date;
        $time = $this->time;
    ...
    public function addfaq()
    {
        ...
        $number = rtrim(filter($_POST['number']));
        $tipe = rtrim(filter($_POST['tipe']));
        $title = rtrim(filter($_POST['title']));
        $konten = rtrim(filter($_POST['konten']));
        $date = $this->date;
    ...
    public function getlayanan()
    {
        ...
        $data['saldopusat'] = $this->model('Digiflazz_model')->ceksaldo();
    ...
    public function deletekategori($code)
    {
        ...
        $this->dbh->query("DELETE FROM kategori_layanan WHERE server = '$tipe'");
        $this->dbh->execute();
        if ($this->dbh->rowCount() > 0) {
            $_SESSION['hasil'] = [
                'alert' => 'success',
    ...
    public function settinguntung()
    {
        $data['title'] = 'Setting keuntungan harga';
        $data['user'] = $this->model('Home_model')->datauser($_SESSION['user']['username']);
        //
        $data['datakeuntunganharga'] = $this->model('Admin_model')->keuntunganharga();
        $this->view('templates/header_admin', $data);
        $this->view('admin/manageprice', $data);
        $this->view('templates/footer_admin');
    }

    public function settinghargauntung()
    {
        if ($this->token->validasi($_POST['token']) == false) {
            die('error, csrf token vailed');
        }
        $id = rtrim(filter($_POST['id_hargauntung']));
        $keuntungan = rtrim(filter($_POST['hargauntung']));
        $upd = $this->db->prepare("UPDATE setting_harga_untung SET harga = '$keuntungan' WHERE id = '$id' ");
        $upd->execute();

        $_SESSION['hasil'] = [
            'alert' => 'success',
            'pesan' => 'Keuntungan berhasil diubah'
        ];

        header('Location:' . BASEURL . 'admin/settinguntung');
    }
}
