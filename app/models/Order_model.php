...
class Order_model
{
    ...
    public function insertorderppob($order_id, $provider_oid, $namalayanan, $harga, $target, $provider, $from, $username  = 'ya', $status)
    {
        ...
        $this->dbh->query("INSERT INTO pembelian_pulsa VALUES ('',:order_id, :provider_id, :username, :namalayanan, '$harga',  :target, '', '$status', '$date', '$time', '$from', '$provider', '0')");
        // bind, 
        $this->dbh->bind('order_id', $order_id);
        $this->dbh->bind('provider_id', $provider_oid);
        $this->dbh->bind('username', $username);
        $this->dbh->bind('namalayanan', $namalayanan);
        $this->dbh->bind('target', $target);
    ...
    public function ceklayananpascabayar($id = '')
    {

        $this->dbh->query("SELECT * FROM layanan_pascabayar WHERE service_id = '$id'");
        $this->dbh->execute();
        return $this->dbh->single();
    }
}
