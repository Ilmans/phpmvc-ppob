...
class Admin_model
{
    ...
    public function totaluser()
    {
        ...
        $this->dbh->query("SELECT count(*) FROM users WHERE status = 'Active' ORDER BY id ASC");
        return $this->dbh->hitungBaris2();
    }
    public function countsaldousers()
    {
        $this->dbh->query("SELECT SUM(saldo_top_up) AS total FROM users WHERE status = 'Active'");
    ...
    {
        $provider = rtrim(filter(($data['provider'])));
        $catatan = rtrim(filter(($data['catatan'])));
        $rate = rtrim(filter(($data['rate'])));
        $nama_penerima = rtrim(filter(($data['nama_penerima'])));
        $tujuan = rtrim(filter(($data['tujuan'])));
        $tipe = rtrim(filter(($data['tipe'])));
        $minimal = rtrim(filter(($data['minimal'])));
        $status = rtrim(filter(($data['status'])));
    ...
    public function keuntunganharga()
    {
        $this->dbh->query("SELECT * FROM setting_harga_untung ORDER BY id DESC");
        $this->dbh->execute();
        return $this->dbh->resultSet();
    }
    // gopay
