...
class Deposit_model
{
    ...
    {
        $this->dbh->query("SELECT * FROM deposit WHERE username = '$username' ORDER BY date DESC");
        $this->dbh->execute();
    ...
    public function riwayatredem($username)
    {
        ...
        $this->dbh->query("SELECT * FROM kode_voucher WHERE user_redem = '$username' AND status = 'Nonaktif' ORDER BY tanggal_redem DESC");
        $this->dbh->execute();
        return $this->dbh->resultSet();
    }
