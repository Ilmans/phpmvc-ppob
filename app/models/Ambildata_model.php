...
class Ambildata_model
{
    ...
    {
        $this->dbh->query("SELECT count(*) FROM kategori_layanan WHERE kategori = '$kategori' ");
        $this->dbh->execute();
    ...
    {
        $this->dbh->query("SELECT count(*) FROM kategori_layanan WHERE kategori = '$category' AND tipe = '$type2'");
        $this->dbh->execute();
        return $this->dbh->hitungBaris2();
    }
    public function inputkategorippob($category, $brand, $type2)
    {
        $this->dbh->query("INSERT INTO kategori_layanan VALUES (null,'$category','$brand','$type2','Top Up')");
        $this->dbh->execute();
        return $this->dbh->rowCount();
    }
    ...
    public function inputlayananppob($sid, $category, $service, $description, $price, $ht_status, $type2, $kategori)
    {
        ...
        $this->dbh->query("INSERT INTO layanan_pulsa VALUES(null, '$sid', '$sid', '$category', '$service', '$description', '$hargaweb', '$hargaapi', '$status', 'DG-PULSA', '$type2', '$kategori')");
        $this->dbh->execute();
        return $this->dbh->rowCount();
    }
    ...
    public function cekorderppob($id)
    {
        $this->dbh->query("SELECT * FROM pembelian_pulsa WHERE oid = '$id' ");
        $this->dbh->execute();
        return $this->dbh->single();
    }
