...
class Home_model
{
    ...
    {
        $this->dbh->query("SELECT * FROM semua_pembelian WHERE user = '$data' ORDER BY date DESC LIMIT 10");
    ...
    {
        $this->dbh->query("SELECT * FROM kategori_layanan WHERE tipe = 'Sosial Media' ORDER BY kategori ASC");
        $this->dbh->execute();
        return $this->dbh->resultSet();
    }
}
